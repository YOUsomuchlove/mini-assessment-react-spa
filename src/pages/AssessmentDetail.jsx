import { ArrowLeftOutlined, ClockCircleOutlined, FormOutlined, PlusOutlined, ReloadOutlined, SearchOutlined, SendOutlined, UserOutlined } from '@ant-design/icons';
...
export default function AssessmentDetail() {
  ...
  const [questionModalOpen, setQuestionModalOpen] = useState(false); const [answerModalOpen, setAnswerModalOpen] = useState(false); const [selectedQuestion, setSelectedQuestion] = useState(null); const [questionSort, setQuestionSort] = useState('default'); const [questionFilter, setQuestionFilter] = useState('all'); const [questionSearch, setQuestionSearch] = useState('');
  ...
  const resetQuestionControls = () => { setQuestionSearch(''); setQuestionFilter('all'); setQuestionSort('default'); };
  ...
  const visibleQuestions = questions.filter((question) => {
    if (questionSearch.trim() && !question.content.replace(/<[^>]*>/g, '').toLowerCase().includes(questionSearch.trim().toLowerCase())) return false;
  ...
  const sortedQuestions = [...visibleQuestions].sort((first, second) => {
    if (questionSort === 'newest') return parseWpUtcDate(second.created_at) - parseWpUtcDate(first.created_at);
    if (questionSort === 'oldest') return parseWpUtcDate(first.created_at) - parseWpUtcDate(second.created_at);
